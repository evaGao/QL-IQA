...
out = fopen('outputs_vgg/out_45.txt', 'wt');
if(out == -1)
...
% Compute LCC and SROCC for the outputs of all nets
    ...
    outFile = fopen(strcat('outputs_vgg/output1', '.txt'),'r');
    outputPatchScores = fscanf(outFile, '%f');
    fclose(outFile);

    % Read score file for image patches into an array (ground truth)
    gTruthFile = fopen(strcat('outputs_vgg/scores1', '.txt'),'r');
    ...
    pCorrs = corr(outputImageScores, gTruthImageScores, 'type', 'Pearson');
    sCorrs = corr(outputImageScores, gTruthImageScores, 'type', 'Spearman');
