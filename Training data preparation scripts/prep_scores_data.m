...
% output directory 
outputDir = strcat(distDataDir, 'map/');
...
% load dmos scores
dmos = load(strcat(distDataDir, 'dmos_all.mat'));
...
% versions
mappingFile = fopen(strcat(distDataDir, 'info_half.txt'), 'r');
if(mappingFile == -1)
...
%            nPatches = calcNumPatches(distImg, patchSize);
            ...
            if(isTrain)
                writescore(scoresFileTrain, distImgName,dmosArray(distImgId));
            else
                writescore(scoresFileVal, distImgName,dmosArray(distImgId));
            end
        end
    end
