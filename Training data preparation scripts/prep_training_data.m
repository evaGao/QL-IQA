...
% Output images' path
outputDir = strcat(inputDir, 'prepared_end/');
...
%    imagePatches = getImagePatches(image, patchSize);
   % imageresize = imresize(image,[256,256],'bilinear');
 ...
 %   normalizedPatches = normalizeLocalContrast(imagePatches, windowSize, patchSize);
  %  normalizedImage= normalized(imageresize, windowSize);
    ...
    save(image, outputDir, imageName);
end
